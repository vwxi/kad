...
impl Store {
    ...
    pub(crate) fn create_new_entry(&self, data: Value) -> StoreEntry {
        ...
        let entry = Entry {
            value: data.clone(),
            signature: node.crypto.sign(serde_json::to_string(&data).unwrap().as_str()),
            origin: kad.as_single_peer(),
            timestamp: timestamp()
        };

        let signature = node.crypto.sign(serde_json::to_string(&entry).unwrap().as_str());
    ...
    pub(crate) fn forward_entry(&self, mut entry: StoreEntry) -> StoreEntry {
        ...
        let signature = node.crypto.sign(serde_json::to_string(&entry.0).unwrap().as_str());
    ...
    pub(crate) async fn put(&self, sender: SinglePeer, entry: StoreEntry) -> bool {
        ...
        if let Value::ProviderRecord(ProviderRecord { provider: _, expiry: e }) = entry.0.value {
            if ts > e {
                return false;
            }
        }
